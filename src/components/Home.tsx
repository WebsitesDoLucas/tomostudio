...
const HeroSection = () => {
  ...
  const xLogo = useTransform(isMobile ? mouseX : mouseXSpring, [-0.5, 0.5], ["-30px", "30px"]);
  const yLogo = useTransform(isMobile ? mouseY : mouseYSpring, [-0.5, 0.5], ["-30px", "30px"]);
  const xBack = useTransform(isMobile ? mouseX : mouseXSpring, [-0.5, 0.5], ["50px", "-50px"]);
  const yBack = useTransform(isMobile ? mouseY : mouseYSpring, [-0.5, 0.5], ["50px", "-50px"]);
  ...
  return (
    <section ref={containerRef} id="intro" onMouseMove={handleMouseMove} className="relative h-screen min-h-[700px] w-full bg-white flex flex-col items-center justify-center px-6 overflow-hidden">
      <motion.div className="absolute inset-0 pointer-events-none" style={{ x: xBack, y: yBack }}>
