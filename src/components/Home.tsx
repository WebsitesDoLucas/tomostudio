...
// 🌟 HOOK UTILITÁRIO: Deteta se é mobile/safari tátil para desativar o que causa lag
const useIsMobile = () => {
...
const CustomCursor = () => {
  ...
  useEffect(() => {
    if (isMobile) return; // 🌟 CANCELA O LOOP NO SAFARI MÓVEL
...
const HeroSection = () => {
  ...
  return (
    <section ref={containerRef} id="intro" onMouseMove={handleMouseMove} className="relative h-screen min-h-[600px] w-full bg-white flex flex-col items-center justify-center px-6 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center text-center transform-gpu">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-4 md:mb-6">
        ...
        <div className="flex flex-col items-center leading-[0.85]">
          <div className="overflow-hidden p-2">
            <motion.h1 initial={{ y: "110%" }} animate={{ y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="text-[14vw] lg:text-[11vw] font-black tracking-tighter" style={{ color: tomoNavy }}>
              CRIAMOS
            </motion.h1>
          </div>
          <div className="overflow-hidden flex items-center justify-center gap-2 md:gap-6 mt-[-2vw] lg:mt-[-1.5vw] p-2 pr-6">
            <motion.h1 initial={{ y: "110%" }} animate={{ y: 0 }} transition={{ duration: 0.8, delay: 0.05, ease: [0.16, 1, 0.3, 1] }} className="text-[14vw] lg:text-[11vw] font-black tracking-tighter" style={{ color: tomoNavy }}>
              CONTIGO
            </motion.h1>
            <div className="relative w-[14vw] h-[14vw] md:w-[10vw] md:h-[10vw] lg:w-[9vw] lg:h-[9vw] p-4 mb-[2vw] flex items-center justify-center">
              <img src={logowebp} alt="Tomo Logo" decoding="async" className="w-full h-full object-contain" />
            </div>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }} className="mt-8">
          <a href="#contacto" className="group flex items-center gap-3 px-8 py-4 rounded-full text-white font-bold text-sm shadow-xl bg-[#020224]">
            Iniciar Projeto
...
// ============================================
// WORKS SECTION (Zero Lag no Safari)
// ============================================
const WorksSection = () => {
  ...
  return (
    <section id="trabalhos" ref={containerRef} className="relative py-16 lg:py-24 bg-white overflow-hidden">
      <motion.div style={{ scale }} className="max-w-[1400px] mx-auto px-6 lg:px-12 transform-gpu">
                ...
                <TransitionReveal direction={index % 2 === 0 ? 'right' : 'left'}>
                  <div className="relative">
                    <div className="relative aspect-[4/3] rounded-[2rem] overflow-hidden shadow-2xl bg-gray-50 transform-gpu">
                      {/* 🌟 OTIMIZAÇÃO: Removemos o Parallax pesado se for telemóvel */}
                      <motion.img 
                        style={{ y: isMobile ? 0 : project.parallax, scale: isMobile ? 1 : 1.12 }} 
                        src={project.image} 
                        alt={project.title} 
                        decoding="async" 
                        loading="eager" 
          ...
          <div className="relative h-full flex items-center justify-center">
            <TransitionReveal direction="right">
              <div className="aspect-[4/3] w-full rounded-[2rem] overflow-hidden border border-black/5 shadow-xl relative z-10 bg-gray-100">
                <motion.img style={{ y: isMobile ? 0 : imageY }} src={fotocasal} alt="Marta e Lucas - tomo studio" decoding="async" className="w-full h-full object-cover object-center" />
              </div>
            </TransitionReveal>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-12 max-w-5xl mx-auto">
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                {[
                  ...
                  <a key={item.title} href={item.href} target={item.href?.startsWith('http') ? "_blank" : undefined} rel={item.href?.startsWith('http') ? "noopener noreferrer" : undefined} className="flex items-start gap-4 cursor-pointer">
          ...
          <TransitionReveal direction="right">
            <form name="contacto" method="POST" data-netlify="true" onSubmit={handleSubmit} className="space-y-5">
              ...
              <button type="submit" disabled={formStatus === 'loading'} className={`w-full px-8 py-4 text-white font-medium text-sm rounded-full transition-all ${formStatus === 'success' ? 'bg-green-500' : formStatus === 'error' ? 'bg-red-500' : 'bg-gradient-to-r from-tomo-blue to-tomo-pink'}`}>
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-10">
          <div className="lg:col-span-2">
            <img src={LogoCompleto} alt="Tomo Studio" loading="lazy" decoding="async" className="h-8 w-auto mb-4 object-contain brightness-0 invert opacity-90" />
          </div>
          <div>
